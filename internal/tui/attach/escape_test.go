...
func TestEscapeDetector_SwallowsSingleEscape(t *testing.T) {
	d := NewEscapeDetector(DefaultEscapeByte, DefaultTimeout)
	pass, detach := d.FeedChunk([]byte{0x1d}, fixedNow)
	if detach {
		t.Fatal("expected no detach")
	}
	if len(pass) != 0 {
...
func TestEscapeDetector_DetachOnTwoConsecutiveInOneChunk(t *testing.T) {
	d := NewEscapeDetector(DefaultEscapeByte, DefaultTimeout)
	_, detach := d.FeedChunk([]byte{0x1d, 0x1d}, fixedNow)
	if !detach {
...
func TestEscapeDetector_NoDetachIfNonEscapeBetween(t *testing.T) {
	d := NewEscapeDetector(DefaultEscapeByte, DefaultTimeout)
	t0 := fixedNow
	d.FeedChunk([]byte{0x1d}, t0)                    // first escape
	d.FeedChunk([]byte{0x61}, within(t0, 50))        // non-escape resets window
	_, detach := d.FeedChunk([]byte{0x1d}, within(t0, 100)) // new first escape
	if detach {
...
func TestEscapeDetector_DropsBytesAfterMidChunkEscapePair(t *testing.T) {
	...
	pass, detach := d.FeedChunk([]byte{0x61, 0x62, 0x1d, 0x1d, 0x63, 0x64}, fixedNow)
	if !detach {
...
func TestEscapeDetector_CustomEscapeByte(t *testing.T) {
	...
	// Default 0x1d should NOT trigger detach with custom escape byte.
	_, det1 := d.FeedChunk([]byte{0x1d, 0x1d}, fixedNow)
	if det1 {
		t.Fatal("0x1d should not detach when escape byte is 0x1c")
	}
...
func TestEscapeDetector_FeedSingleByte(t *testing.T) {
	...
	pass, det := d.Feed(0x1d, t0)
	if det || len(pass) != 0 {
		t.Fatal("first escape: expect swallow, no detach")
	}

	pass2, det2 := d.Feed(0x1d, within(t0, 100))
	if !det2 || len(pass2) != 0 {
